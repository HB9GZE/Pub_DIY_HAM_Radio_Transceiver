...
err_t si5351_Init(void)
{
	...
	ASSERT_STATUS(si5351_write8(SI5351_REGISTER_183_CRYSTAL_INTERNAL_LOAD_CAPACITANCE, m_si5351Config.crystalLoad));
...

{
	...
	while (HAL_I2C_IsDeviceReady(&hi2c4, (uint16_t) (SI5351_ADDRESS << 1), 3, 100) != HAL_OK)
	{
	}

	status = HAL_I2C_Mem_Write(&hi2c4,							// i2c handle
...
err_t si5351_read8(uint8_t reg, uint8_t *value)
{
	...
	status = HAL_I2C_Mem_Read(&hi2c4,							// i2c handle
			(uint8_t) (SI5351_ADDRESS << 1),		// i2c address, left aligned
			(uint8_t) reg,						// register address
			I2C_MEMADD_SIZE_8BIT,		// si5351 uses 8bit register addresses
			(uint8_t*) (&value),		// write returned data to this variable
